...
<body>
	...
	<div class="shoplist-wrap">
		<c:forEach items="${product_zuqiu}" var="p">
			商品名称:<span>${p.name}</span><br>
			图片:<input type="image" name="listimg" src="images/足球/${p.listimg}" width="180" height="150" /><br>
			<div class="showprice">
			原价:<span>${p.price}</span>
			折后价:<span>${p.discountprice}</span><br>
			</div>
			<div class="delandupade">
			<span><a href="ProductDeleteServlet?id=${p.id }">删除</a></span>
			<span><a href="ProductSelectServlet?id=${p.id }">修改</a></span><br><br>
			</div>
		</c:forEach>
	</div>
